#pragma once
#include <math.h>
#include <iostream>
#include "BusGraph.h"
using namespace std;

#ifndef HEAPHPP
#define HEAPHPP

class Heap {
 protected:
  BusGraphEdge* _heap;
  int *_index_table;
  int heap_size;

 public:
  Heap(int num_of_nodes) { 
    _heap = new BusGraphEdge[num_of_nodes]; 
    _index_table = new int[num_of_nodes];
    heap_size = 0;

    for(int i = 0 ; i < num_of_nodes ; i++){
      _index_table[i] = -1;
    }
  }

  int size() const {
    return heap_size;
  }

  bool empty() const {
    if(heap_size <= 0){
      return true;
    }
    return false;
  }

  void insert(const BusGraphEdge&);
  BusGraphEdge extractMin();
  BusGraphEdge peekMin() const;
  void printHeapArray() const;
  void changeKey(const BusGraphEdge& from, const BusGraphEdge& to);
  void deleteItem(const BusGraphEdge&);
  BusGraphEdge operator[](int);

  ~Heap() { 
    delete[] _heap; 
    delete[] _index_table;
  };

  void bubble_up(int index){
    int new_index = (index-1)/2;

    if(new_index < 0 || new_index >= heap_size){
      return;
    }
    
    if(_heap[index] < _heap[new_index]){
      swap(new_index,index);
      bubble_up(new_index);
    }
  }

  void swap(int index1 , int index2){
    BusGraphEdge e1 = _heap[index1];
    BusGraphEdge e2 = _heap[index2]; 
    _heap[index2] = e1;
    _heap[index1] = e2;
    _index_table[e1.dest()] = index2;
    _index_table[e2.dest()] = index1;
  }

  void bubble_down(int index){

    int new_index = 0;
    bool left = false;
    bool right = false;

    if(index*2+1 < heap_size){
      left = (_heap[index*2+1] < _heap[index]);
    }
    if(index*2+2 < heap_size){
      right = (_heap[index*2+2] < _heap[index]);
    }
    if(left && right){
      if(_heap[index*2+1] < _heap[index*2+2]){
        new_index = index*2+1;
      }else{
        new_index = index*2+2;
      }
      swap(index,new_index);
      bubble_down(new_index);
    
    }else if(left){
      new_index = index*2+1;
      swap(index,new_index);
      bubble_down(new_index);
    }else if(right){
      new_index = index*2+2;
      swap(index,new_index);
      bubble_down(new_index);
    }
  }
};

void Heap::insert(const BusGraphEdge& item){
  _heap[heap_size] = item;
  _index_table[item.dest()] = heap_size;
  bubble_up(heap_size);
  heap_size++;
}

BusGraphEdge Heap::extractMin() {
    if(heap_size == 0){
      throw std::out_of_range("no elements in heap");
    }
    BusGraphEdge temp = _heap[0];
    swap(0,heap_size-1);
    //removes edge
    _heap[heap_size-1] = BusGraphEdge();
    _index_table[temp.dest()] = -1;
    heap_size--;
    bubble_down(0);
    return temp;
}

BusGraphEdge Heap::peekMin() const {
  if(heap_size <= 0){
    throw std::out_of_range("empty heap");
  }
  return _heap[0];
};


void Heap::printHeapArray() const {
  for (int i = 0; i < size(); i++) {
    cout << _heap[i] << " ";
  }
  cout << endl;
}

void Heap::changeKey(const BusGraphEdge& from, const BusGraphEdge& to) {
  // TODO: implement this
  int index = _index_table[from.dest()];
  if(index == -1){
    throw std::out_of_range("no such element");
  }
  _index_table[from.dest()] = -1;
  _heap[index] = to;
  _index_table[to.dest()] = index;

  int new_index = (index-1)/2;

  if(new_index >= 0 && _heap[index] < _heap[new_index]){
    bubble_up(index);
  }else{
    bubble_down(index);
  }
}

void Heap::deleteItem(const BusGraphEdge& x) {
  int index = _index_table[x.dest()];
  if(index == -1){
    throw std::out_of_range("no such element");
  }
  swap(index , heap_size-1);
  _heap[heap_size-1] = BusGraphEdge();
  _index_table[x.dest()] = -1;
  heap_size--;
  int new_index = (index-1)/2;
 
  if(new_index >= 0 && _heap[index] < _heap[new_index]){
    bubble_up(index);
  }else{
    bubble_down(index);
  }
}

BusGraphEdge Heap::operator[](int dest){
  return _heap[_index_table[dest]];
}

#endif
